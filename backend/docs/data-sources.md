...
| **SIVIGILA reciente y operativo 2025+** | **No hay API pública tabular nacional estable confirmada** | [Buscador de microdatos SIVIGILA](https://portalsivigila.ins.gov.co/buscador), [vigilancia rutinaria](https://portalsivigila.ins.gov.co/Paginas/Vigilancia-Rutinaria.aspx) y BES | El BES se publica semanalmente, pero no constituye una base municipal completa. Al 14-07-2026 PRORA no verificó archivos anuales nacionales 2025+ con el contrato anterior. | El esquema depende del evento y del diccionario anual SIVIGILA; el BES es PDF | Aplicar condiciones del INS y finalidad declarada | No se automatiza una sesión humana ni se infieren municipios desde totales departamentales. La federación territorial (abajo) **extiende** cobertura local reciente; no sustituye un SIVIGILA nacional operativo. |
| **SIVIGILA territorial abierto (federación)** | **Sí, SODA 2.1 por miembro** | Federación `sivigila-territorial-open`: Boyacá [`tz38-fg9k`](https://www.datos.gov.co/resource/tz38-fg9k.json), Caquetá dengue [`fhz2-4x64`](https://www.datos.gov.co/resource/fhz2-4x64.json), Pereira [`tm62-e28n`](https://www.datos.gov.co/resource/tm62-e28n.json), Tuluá [`dq2k-gub9`](https://www.datos.gov.co/resource/dq2k-gub9.json), Bucaramanga dengue [`qzc7-jbg3`](https://www.datos.gov.co/resource/qzc7-jbg3.json), Bucaramanga EISP [`map9-mdzc`](https://www.datos.gov.co/resource/map9-mdzc.json), Bucaramanga IRA colectiva [`dtct-ww7w`](https://www.datos.gov.co/resource/dtct-ww7w.json), Casanare [`3kuf-t86h`](https://www.datos.gov.co/resource/3kuf-t86h.json), Santa Rosa de Cabal [`xc7d-5tmm`](https://www.datos.gov.co/resource/xc7d-5tmm.json) | Municipio-semana; ventanas tipicamente 2015–2025. Bucaramanga multi-evento e IRA hasta **2025**. | Códigos/nombres DIVIPOLA, evento, año, semana; filas caso se agregan a conteo | CC BY-SA 4.0 u otras según ficha local | Adapter `territorial_sivigila`. Merge: año más reciente gana; mismo año prioriza territorial. Malaria Caquetá `jkh2-sgg6` fuera (solo mes). |
| **IRCA calidad del agua** | **Sí, SODA 2.1** | [`nxt2-39c3`](https://www.datos.gov.co/resource/nxt2-39c3.json) | Nacional municipal anual **2013–2024** | `municipiocodigo`, `a_o`, `irca`, niveles de riesgo | Atribución INS | Fuente `ins-irca-water-quality`; persiste `irca_index` sin borrar indicadores CNPV. |
| **PAI territorial Valle** | **Sí, SODA 2.1** | [`uw8e-gzpp`](https://www.datos.gov.co/resource/uw8e-gzpp.json) | Municipio-mes (muestra verificada 2022) | `codigo_municipio`, `biologico`, `cobertura`, dosis | Atribución Gobernación del Valle | Fuente `pai-valle-municipal`; complementa ZIP PAI nacional. |
...
| **PAI municipal 2026** | **Sí, archivo público directo** | [`dosis-coberturas-biologicos-municipios-2026.zip`](https://www.minsalud.gov.co/sites/rid/Lists/BibliotecaDigital/RIDE/VS/PP/PAI/dosis-coberturas-biologicos-municipios-2026.zip) | Hojas `Enero`–`Mayo` (corte acumulado verificado 28-07-2026); ~1.147 municipios/hoja; ZIP SHA-256 `27e72159…` | `Codep`, `Departamento`, `COD` (DIVIPOLA), `Municipio`, metas, dosis y coberturas por biológico/grupo | No se observó licencia estructurada junto al ZIP; atribución MinSalud | Contrato `pai-municipal-v1.1` con checksum fijado. Si MinSalud republica el ZIP, actualizar hashes antes de re-sync. |
| **SIVIGILA microdatos 2025** | **Pendiente de publicación** | Plantilla esperada `Datos_2025_{evento}.xlsx` en [Microdatos](https://portalsivigila.ins.gov.co/Microdatos/Forms/AllItems.aspx) | Fuente catalogada `sivigila-microdata-2025` en `REQUIRES_CONFIGURATION` (HTTP 404 al 28-07-2026) | Igual contrato que 2024 cuando existan archivos | Condiciones INS | No inventar series. Cuando INS publique, activar espejo del adaptador 2024. |
| **SIVIGILA autorizado 2025+ (upload)** | **CSV institucional** | Fuente `sivigila-current-authorized` + plantilla `GET /api/v1/sources/templates/epidemiology` | Municipio-semana canónico; requiere rol analyst/admin | `municipality_code,disease,week_start,cases[,population,is_preliminary,quality_score]` | Condiciones de la entidad entregadora | Con `PRORA_JOBS_INLINE=true` el upload se procesa en la API. Luego re-entrenar para elegibilidad operativa si el corte ≤35 días. |
...
| `sivigila-territorial-open` (+ 6 miembros) | **Integrado 2026-07-14** | Federación activa de publicaciones territoriales en datos.gov.co. Extiende series (p. ej. dengue Bucaramanga a 2025). No declara readiness nacional operativo. |
...
1. Mantener `4hyg-wa9d` para 2007–2022, ejecutar microdatos 2024 y sincronizar
   `sivigila-territorial-open` (o miembros) para extender municipio-semana en territorios con
   series 2018–2025. Conservar un SIVIGILA nacional 2025+ como fuente condicionada hasta
   verificar publicación tabular oficial; el BES no se asigna a municipios.
