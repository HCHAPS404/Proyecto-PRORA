...
export default function Dashboard({ onOpenAlerts, onOpenData, onNotify }: DashboardProps) {
  ...
  const scenarioForecastMode = useMemo(
    () => riskItems.length > 0 && riskItems.every((item) => item.forecast_mode === 'scenario_projection'),
    [riskItems],
  )
  const researchForecastMode = useMemo(
    () => riskItems.length > 0 && riskItems.every((item) => item.forecast_mode === 'retrospective_research' || item.forecast_mode === 'scenario_projection' || item.operationally_eligible === false),
    [riskItems],
  )
  ...
  const observedPriority = useMemo(
    () => [...observedTerritories]
      .sort((a, b) => b.latest_observed_cases - a.latest_observed_cases || b.total_observed_cases - a.total_observed_cases)
      .slice(0, 6),
    [observedTerritories],
  )
  const observedCoverage = useMemo(() => {
    if (!observedTerritories.length) return null
    const totalCases = observedTerritories.reduce((sum, item) => sum + item.total_observed_cases, 0)
    const latestWeek = observedTerritories.reduce((latest, item) => (
      !latest || item.latest_week > latest ? item.latest_week : latest
    ), '')
    const firstWeek = observedTerritories.reduce((earliest, item) => (
      !earliest || item.first_week < earliest ? item.first_week : earliest
    ), '')
    return { totalCases, latestWeek, firstWeek, municipalities: observedTerritories.length }
  }, [observedTerritories])
  ...
  const latestCasesDisplay = summary?.latest?.observed_cases ?? selectedObservedTerritory?.latest_observed_cases ?? null
  const latestWeekDisplay = summary?.latest?.week ?? selectedObservedTerritory?.latest_week ?? null
  const latestCasesDetail = latestWeekDisplay
    ? `${territoryContext} · ${formatDate(latestWeekDisplay)}${summary?.data_status === 'fresh' ? ' · vigente' : summary?.data_status === 'stale' ? ' · rezagado' : ' · observado'}`
    : summaryState === 'offline' && !selectedObservedTerritory
  ...
  const territorySelectOptions = useMemo(() => [
    ...riskItems.map((item) => ({
      ...
      group: scenarioForecastMode ? 'Proyección de escenario (histórico → 2026)' : researchForecastMode ? 'Con pronóstico del modelo (retrospectivo)' : 'Con pronóstico operativo',
  ...
  ], [historicalOnlyTerritories, researchForecastMode, scenarioForecastMode, riskItems])
  ...
  return (
    <div className="dashboard workspace-view">
      <header className="dashboard-heading">
        <div><div className="heading-meta"><span className={`status-dot${apiResponded ? ' status-dot--online' : ''}`} /> {summaryState === 'loading' ? 'Consultando fuentes epidemiológicas…' : !apiResponded ? 'Backend no disponible' : summary?.latest || selectedObservedTerritory ? `Último corte observado: ${formatDate(latestWeekDisplay)}` : 'API conectada · sin observaciones para este evento'}</div><h1>Panorama nacional</h1><p>Observaciones históricas y señales operativas de {disease.label}, diferenciadas por vigencia y procedencia.</p></div>
      ...
      <section className="metric-grid" aria-label="Indicadores principales">
        <MetricCard title="Casos · último corte territorial" value={summaryState === 'loading' && !selectedObservedTerritory ? '…' : formatNumber(latestCasesDisplay)} detail={latestCasesDetail} icon={CalendarDays} featured />
        <MetricCard title="Variación frente al corte anterior" value={weeklyVariation == null ? '—' : `${weeklyVariation >= 0 ? '+' : ''}${formatNumber(weeklyVariation)}%`} detail={summary?.previous ? `${territoryContext} · frente a ${formatDate(summary.previous.week)}` : selectedObservedTerritory ? `${formatNumber(selectedObservedTerritory.observation_rows)} cortes en la serie` : 'Sin dos cortes comparables'} icon={CircleGauge} />
        <MetricCard title="Casos acumulados · 4 semanas" value={summaryState === 'loading' && !selectedObservedTerritory ? '…' : formatNumber(fourWeekWindow?.observed_cases ?? selectedObservedTerritory?.total_observed_cases)} detail={fourWeekWindow ? `${fourWeekWindow.observed_week_count}/4 semanas con reporte · no se imputan ceros` : selectedObservedTerritory ? `Total histórico: ${formatNumber(selectedObservedTerritory.total_observed_cases)} casos` : 'Sin ventana territorial disponible'} icon={MapPin} />
        <MetricCard title="Alertas operativas vigentes" value={alertsState === 'loading' ? '…' : alertsState === 'offline' ? '—' : formatNumber(operationalAlerts.length)} detail={alertsState === 'offline' ? 'API de alertas no disponible' : operationalAlerts.length ? `${disease.label} · horizonte ${horizon} semanas` : observedCoverage ? `${formatNumber(observedCoverage.municipalities)} municipios con historia` : retrospectiveAlerts.length ? `${retrospectiveAlerts.length} registros históricos, no vigentes` : 'Sin alertas vigentes publicadas'} icon={ShieldAlert} />
      </section>

      {readingMode === 'simple' ? (
        <section className="simple-reading-summary" aria-label="Lectura simple del territorio">
          <Sparkles size={18} />
          <div><strong>Lectura rápida de {territoryContext}</strong><span>{summary?.latest ? `El último corte municipal notificó ${formatNumber(summary.latest.observed_cases)} casos; la ventana de cuatro semanas acumula ${formatNumber(fourWeekWindow?.observed_cases)}.` : selectedObservedTerritory ? `Último corte observado ${formatDate(selectedObservedTerritory.latest_week)} con ${formatNumber(selectedObservedTerritory.latest_observed_cases)} casos; la serie publica ${formatNumber(selectedObservedTerritory.observation_rows)} cortes y ${formatNumber(selectedObservedTerritory.total_observed_cases)} casos acumulados.` : 'No hay observaciones municipales suficientes para resumir este territorio.'} {summary?.data_status === 'stale' ? `El corte tiene ${formatNumber(summary.observation_age_days)} días de rezago y no debe interpretarse como situación actual.` : ''} {observedCoverage ? ` Cobertura cargada: ${formatNumber(observedCoverage.municipalities)} municipios · ${formatDate(observedCoverage.firstWeek)} → ${formatDate(observedCoverage.latestWeek)}.` : ''} {currentReference ? ` Como referencia reciente separada, el INS informa ${formatNumber(currentReference.cumulative_cases)} casos acumulados en ${currentReference.reference_territory_name} hasta la semana epidemiológica ${currentReference.epidemiological_week} de ${currentReference.epidemiological_year}; esta referencia no reemplaza el corte municipal.` : ''} {!model ? ' Para predicción IA y factores del modelo: sincronice más fuentes y entrene un champion.' : ''}</span></div>
        </section>
      ) : (
      ...
      <section className="dashboard-primary-grid">
        ...
        <aside className="content-card priority-panel">
          <div className="card-heading-row"><div><span className="eyebrow">{operationalAlerts.length ? 'Alertas operativas' : observedPriority.length ? 'Prioridad observada' : 'Archivo de alertas'}</span><h2>{operationalAlerts.length ? 'Prioridad territorial vigente' : observedPriority.length && !visiblePriorityAlerts.length ? 'Municipios con más casos recientes' : 'Historial verificable'}</h2></div><ShieldAlert size={20} /></div>
          <p className="card-description">{operationalAlerts.length ? 'Orden vigente devuelto por el backend para el evento seleccionado.' : visiblePriorityAlerts.length ? 'Registros emitidos anteriormente; no representan riesgo actual.' : observedPriority.length ? 'Ranking por último corte observado (no es riesgo del modelo). Entrene un champion para alertas predictivas.' : 'No existen alertas emitidas para este filtro.'}</p>
          {visiblePriorityAlerts.length ? <div className="priority-list">{visiblePriorityAlerts.slice(0, 6).map((alert, index) => <button key={alert.id} className={selectedCode === alert.cod_dane ? 'priority-item is-active' : 'priority-item'} onClick={() => setSelectedCode(alert.cod_dane)}><span className={`priority-rank priority-rank--${alert.risk_level.replace('í', 'i')}`}>{String(index + 1).padStart(2, '0')}</span><span className="priority-item__copy"><strong>{alert.municipality}</strong><small>{alert.department} · {titleCase(alert.disease)}</small><em>{alert.operationally_eligible ? alert.status : `Histórica · objetivo ${formatDate(alert.target_week)}`}</em></span><span className="priority-item__score"><strong>{formatNumber(alert.risk_score)}</strong><small>/ 100</small><em>{formatNumber(alert.predicted_cases)} casos estimados</em></span></button>)}</div> : observedPriority.length ? <div className="priority-list">{observedPriority.map((item, index) => <button key={item.cod_dane} className={selectedCode === item.cod_dane ? 'priority-item is-active' : 'priority-item'} onClick={() => setSelectedCode(item.cod_dane)}><span className="priority-rank priority-rank--moderado">{String(index + 1).padStart(2, '0')}</span><span className="priority-item__copy"><strong>{item.municipality}</strong><small>{item.department} · observado</small><em>Último corte {formatDate(item.latest_week)}</em></span><span className="priority-item__score"><strong>{formatNumber(item.latest_observed_cases)}</strong><small>casos</small><em>{formatNumber(item.observation_rows)} cortes · {formatNumber(item.total_observed_cases)} acum.</em></span></button>)}</div> : <div className="empty-state">{alertsState === 'loading' ? <LoaderCircle className="spin" size={26} /> : <ShieldAlert size={26} />}<h3>{alertsState === 'loading' ? 'Consultando alertas' : alertsState === 'offline' ? 'API de alertas no disponible' : 'Sin alertas registradas'}</h3><p>{alertsState === 'offline' ? 'No se muestran alertas locales de respaldo.' : 'El backend no contiene alertas vigentes ni históricas para este evento.'}</p></div>}
