...
export default function AlertsCenter({ onOpenAlert, onNotify }: AlertsCenterProps) {
  ...
  const [observedSignals, setObservedSignals] = useState<HistoricalTerritory[]>([])
  const [loadingObserved, setLoadingObserved] = useState(false)
  const [observedDisease, setObservedDisease] = useState('dengue')
  ...
  useEffect(() => {
    if (loadingAlerts || alertItems.length > 0 || Boolean(alertsError)) {
      setObservedSignals([])
      return
    }
    let active = true
    setLoadingObserved(true)
    proraApi.analytics.historicalTerritories(observedDisease)
      .then((result) => {
        if (!active) return
        const ranked = [...result.items]
          ...
          .slice(0, 12)
        setObservedSignals(ranked)
      })
      .catch(() => { if (active) setObservedSignals([]) })
      .finally(() => { if (active) setLoadingObserved(false) })
    return () => { active = false }
  }, [alertItems.length, alertsError, loadingAlerts, observedDisease])
  ...
  const formatObservedDate = (value: string) => new Intl.DateTimeFormat('es-CO', { dateStyle: 'medium' }).format(new Date(/^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T00:00:00` : value))
  const showObservedFallback = !loadingAlerts && !alertsError && alertItems.length === 0
  const observedCasesTotal = observedSignals.reduce((sum, item) => sum + item.latest_observed_cases, 0)
  ...
  return (
    <section className="workspace-view alerts-center" aria-labelledby="alerts-title">
      ...
      <div className="alerts-layout">
        <div className="alerts-main-column"><article className="content-card alert-feed-card"><div className="card-heading-row alert-feed-heading"><div><h2>{showObservedFallback ? 'Prioridad observada' : 'Alertas epidemiológicas'}</h2><p>{showObservedFallback ? 'Misma señal del panorama nacional mientras no haya pronósticos publicados.' : 'Ordenadas por nivel de riesgo predictivo.'}</p></div><span className="live-label"><span /> {loadingAlerts ? 'Consultando' : alertsError ? 'API no disponible' : alertItems.length ? 'Conectado' : showObservedFallback ? 'Histórico' : 'Sin publicar'}</span></div>
          {showObservedFallback ? (
            <>
              <div className="observed-signal-banner">
                <strong>Sin alertas predictivas publicadas</strong>
                <p>El centro de alertas solo lista señales del modelo. Mientras tanto se muestran los municipios con más casos en el último corte — la misma prioridad observada del panorama nacional.</p>
              </div>
              <div className="filter-bar">
                <label className="select-with-icon">
                  <Filter size={16} />
                  <span className="sr-only">Evento observado</span>
                  <select value={observedDisease} onChange={(event) => setObservedDisease(event.target.value)} aria-label="Evento observado">
                    {diseaseOptions.map((item) => <option value={item} key={item}>{item === 'ira' ? 'IRA' : titleCase(item)}</option>)}
                  </select>
                </label>
              </div>
              {loadingObserved && <div className="empty-state"><LoaderCircle className="spin" size={27} /><h3>Consultando prioridad observada</h3><p>Recuperando territorios históricos del panorama.</p></div>}
              {!loadingObserved && observedSignals.length > 0 && (
                <div className="observed-signal-list">
                  {observedSignals.map((item, index) => (
                    <div className="observed-signal-row" key={item.cod_dane}>
                      <span className="priority-rank priority-rank--moderado">{String(index + 1).padStart(2, '0')}</span>
                      <div>
                        <strong>{item.municipality}</strong>
                        <small>{item.department} · DANE {item.cod_dane} · corte {formatObservedDate(item.latest_week)}</small>
                      </div>
                      <span>{item.latest_observed_cases.toLocaleString('es-CO')}<small style={{ display: 'block', fontWeight: 600 }}>casos</small></span>
                    </div>
                  ))}
                </div>
              )}
              {!loadingObserved && observedSignals.length === 0 && (
                <div className="empty-state"><Search size={27} /><h3>Sin prioridad observada</h3><p>No hay territorios históricos publicados para {titleCase(observedDisease)}. Cargue datos y ejecute un entrenamiento para publicar señales predictivas.</p></div>
              )}
            </>
          ) : (
            <>
              <div className="filter-bar"><label className="search-field"><Search size={17} /><span className="sr-only">Buscar alertas</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Municipio, departamento o evento" /></label><label className="select-with-icon"><Filter size={16} /><span className="sr-only">Nivel de riesgo</span><select value={level} onChange={(event) => setLevel(event.target.value as 'Todas' | RiskLevel)}><option>Todas</option><option>Crítico</option><option>Alto</option><option>Moderado</option></select></label><select value={disease} onChange={(event) => setDisease(event.target.value)} aria-label="Filtrar por enfermedad">{diseases.map((item) => <option key={item}>{item}</option>)}</select></div>
              <div className="alerts-table-wrap"><table className="data-table alerts-table"><thead><tr><th>Evento y territorio</th><th>Riesgo</th><th>Proyección</th><th>Principales impulsores</th><th>Actualización</th><th><span className="sr-only">Acciones</span></th></tr></thead><tbody>{visibleAlerts.map((alert) => <tr key={alert.id} className={acknowledged.has(alert.id) ? 'row-acknowledged' : ''}><td><div className="alert-identity"><span className={`disease-mark disease-${alert.disease.toLowerCase()}`}><HeartPulse size={17} /></span><div><strong>{alert.disease}</strong><span><MapPin size={12} /> {alert.municipality}, {alert.department}</span><span><Clock3 size={12} /> {alertStatusDescription(alert)}</span></div></div></td><td><div className="risk-cell"><span className={`risk-pill risk-${alert.level.toLowerCase()}`}>{alert.level}</span><strong>{alert.risk}%</strong></div></td><td><strong>{alert.predictedCases}</strong><span>{alert.horizon}</span></td><td><div className="driver-tags">{alert.drivers.map((driver, index) => <span key={driver}>{index === 0 ? <CloudRain size={12} /> : <Wind size={12} />}{driver}</span>)}</div></td><td><span className="updated-time">{alert.updated}</span></td><td>{reviewAction(alert)}<button className="icon-button" type="button" aria-label={`Abrir alerta de ${alert.disease} en ${alert.municipality}`} onClick={() => openAlertDetail(alert)}><ChevronRight size={17} /></button></td></tr>)}</tbody></table></div>
              <div className="alerts-mobile-list">{visibleAlerts.map((alert) => <article className="mobile-alert-card" key={alert.id}><div className="mobile-alert-heading"><span className={`disease-mark disease-${alert.disease.toLowerCase()}`}><HeartPulse size={17} /></span><div><strong>{alert.disease}</strong><span>{alert.municipality} · {alert.department} · {alertStatusLabel(alert).toLocaleLowerCase('es')}</span></div><span className={`risk-pill risk-${alert.level.toLowerCase()}`}>{alert.level}</span></div><div className="mobile-alert-data"><span>Riesgo <strong>{alert.risk}%</strong></span><span>Casos <strong>{alert.predictedCases}</strong></span><span>Horizonte <strong>{alert.horizon}</strong></span></div><div className="mobile-alert-footer"><span>{alert.updated}</span><button className="button button-ghost button-small" type="button" onClick={() => openAlertDetail(alert)}>Ver detalle <ChevronRight size={14} /></button></div></article>)}</div>
              {visibleAlerts.length === 0 && <div className="empty-state"><Search size={27} /><h3>{loadingAlerts ? 'Consultando el backend' : alertsError ? 'No fue posible cargar las alertas' : 'No hay alertas con estos filtros'}</h3><p>{loadingAlerts ? 'Estamos verificando los pronósticos disponibles.' : alertsError || 'Ajuste la búsqueda para ampliar los resultados.'}</p></div>}
            </>
          )}
        </article></div>
        ...
        <aside className="alerts-side-column">
          ...
          <article className="content-card forecast-callout"><div className="forecast-visual"><Droplets size={25} /><span><Sparkles size={14} /></span></div><span className="eyebrow">Lectura rápida</span><h3>{primaryDriver ? `${titleCase(primaryDriver[0])} concentra las señales actuales` : showObservedFallback && observedSignals[0] ? `${observedSignals[0].municipality} lidera el último corte` : 'Aún no hay una señal nacional publicada'}</h3><p>{primaryDriver ? `Este impulsor aparece en ${primaryDriver[1]} de ${activeAlerts.length} alertas operativas vigentes.` : showObservedFallback && observedSignals[0] ? `${observedSignals[0].latest_observed_cases.toLocaleString('es-CO')} casos en el corte ${formatObservedDate(observedSignals[0].latest_week)}. Entrene un modelo para convertir esta prioridad observada en alertas predictivas.` : 'El resumen se calculará automáticamente con los impulsores de pronósticos validados.'}</p>{showNationalAnalysis && <div className="driver-ranking">{rankedDrivers.slice(0, 5).map(([driver, count]) => <div key={driver}><span>{titleCase(driver)}</span><strong>{count}</strong></div>)}{rankedDrivers.length === 0 && (showObservedFallback ? observedSignals.slice(0, 5).map((item) => <div key={item.cod_dane}><span>{item.municipality}</span><strong>{item.latest_observed_cases.toLocaleString('es-CO')}</strong></div>) : <p>Sin impulsores publicados para analizar.</p>)}</div>}<button className="text-button" type="button" onClick={() => setShowNationalAnalysis((value) => !value)} aria-expanded={showNationalAnalysis}>{showNationalAnalysis ? 'Ocultar análisis' : 'Ver análisis nacional'} <ChevronRight size={15} /></button></article>
        </aside>
      </div>
